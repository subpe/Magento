define(
    ...
    function ($, Component, placeOrderAction, selectPaymentMethodAction, customer, checkoutData, additionalValidators) {
        ...
        $('.errP').text('');
        var promo='';
        /*alert(window.checkoutConfig.payment.bhartipay.hide_promo_field);
        alert($.trim(window.checkoutConfig.payment.bhartipay.promoCode));
        alert($.trim(window.checkoutConfig.payment.bhartipay.promoCode).length);
        alert(window.checkoutConfig.payment.bhartipay.promoCode);*/
        if(window.checkoutConfig.payment.bhartipay.hide_promo_field=='1'){
            if(window.checkoutConfig.payment.bhartipay.promo_code_local_validation=='1' && ($.trim(window.checkoutConfig.payment.bhartipay.promoCode).length=='0' || $.trim(window.checkoutConfig.payment.bhartipay.promoCode)=='')){
        ...
        $(document).on('click','.promoApplyBtn',function(){
            // alert(window.checkoutConfig.payment.bhartipay.promoCode);
            if(window.checkoutConfig.payment.bhartipay.promo_code_local_validation=='1'){
                if($.trim($('.promoCodeField').val())==''){
                    $('.errP').text('Please enter your promo code.');
                }else{
                    var promoCode=window.checkoutConfig.payment.bhartipay.promoCode;
        ...
        return Component.extend({
            defaults: {
                template: 'Bhartipaypg_Bhartipay/payment/bhartipaypg'
            },
            ...
            afterPlaceOrder: function () {
                if(promo==''){
                    $.mage.redirect(window.checkoutConfig.payment.bhartipay.redirectUrl);
                }else{
                    $.mage.redirect(window.checkoutConfig.payment.bhartipay.redirectUrl+"?promo="+promo);
                }
            }
        });
    }
