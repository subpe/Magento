...
use Subpepg\Subpe\Helper\Data as DataHelper;

class Subpe extends \Magento\Payment\Model\Method\AbstractMethod
{
    const CODE = 'subpe';
    ...
    protected $_formBlockType = 'Subpepg\Subpe\Block\Form\Subpe';
    protected $_infoBlockType = 'Subpepg\Subpe\Block\Info\Subpe';
    ...
    public function __construct(
        ...
        \Subpepg\Subpe\Helper\Data $helper
    ) {
        $this->helper = $helper;
        parent::__construct(
            $context,
    ...
    public function buildSubpeRequest($order)
    {
        $callBackUrl=$this->urlBuilder->getUrl('subpe/Standard/Response', ['_secure' => true]);
    ...
    $post_group_data['TXNTYPE']='SALE';
      $post_group_data['HASH']="$hash";
        if(isset($order->subpePromoCode)){
            $params['PROMO_CAMP_ID']=$order->subpePromoCode;
        }
