define(
    ...
    function ($, Component, placeOrderAction, selectPaymentMethodAction, customer, checkoutData, additionalValidators) {
        ...
        $('.errP').text('');
        var promo='';
        /*alert(window.checkoutConfig.payment.subpe.hide_promo_field);
        alert($.trim(window.checkoutConfig.payment.subpe.promoCode));
        alert($.trim(window.checkoutConfig.payment.subpe.promoCode).length);
        alert(window.checkoutConfig.payment.subpe.promoCode);*/
        if(window.checkoutConfig.payment.subpe.hide_promo_field=='1'){
            if(window.checkoutConfig.payment.subpe.promo_code_local_validation=='1' && ($.trim(window.checkoutConfig.payment.subpe.promoCode).length=='0' || $.trim(window.checkoutConfig.payment.subpe.promoCode)=='')){
        ...
        $(document).on('click','.promoApplyBtn',function(){
            // alert(window.checkoutConfig.payment.subpe.promoCode);
            if(window.checkoutConfig.payment.subpe.promo_code_local_validation=='1'){
                if($.trim($('.promoCodeField').val())==''){
                    $('.errP').text('Please enter your promo code.');
                }else{
                    var promoCode=window.checkoutConfig.payment.subpe.promoCode;
        ...
        return Component.extend({
            defaults: {
                template: 'Subpepg_Subpe/payment/subpepg'
            },
            ...
            afterPlaceOrder: function () {
                if(promo==''){
                    $.mage.redirect(window.checkoutConfig.payment.subpe.redirectUrl);
                }else{
                    $.mage.redirect(window.checkoutConfig.payment.subpe.redirectUrl+"?promo="+promo);
                }
            }
        });
    }
