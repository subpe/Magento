...
class Bpay_subpe_Helper_Data extends Mage_Payment_Helper_Data
{
...
function verifychecksum_e($arrayList, $key, $checksumvalue) {
	$arrayList = Mage::helper('subpe')->removeCheckSumParam($arrayList);
	ksort($arrayList);
	$str = Mage::helper('subpe')->getArray2StrForVerify($arrayList);
	$subpe_hash = Mage::helper('subpe')->decrypt_e($checksumvalue, $key);
	$salt = substr($subpe_hash, -4);
...
function getArray2StrForVerify($arrayList) {
	...
	foreach ($arrayList as $key => $value) {
		if ($flag) {
			$paramStr .= Mage::helper('subpe')->checkString_e($value);
			$flag = 0;
		} else {
			$paramStr .= "|" . Mage::helper('subpe')->checkString_e($value);
		}
...
function redirect2PG($paramList, $key) {
	$hashString = Mage::helper('subpe')->getchecksumFromArray($paramList);
	$checksum = Mage::helper('subpe')->encrypt_e($hashString, $key);
}
