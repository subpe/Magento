...
class Bpay_subpe_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'subpe/success';
    protected $_failureBlockType = 'subpe/failure';
    protected $_cancelBlockType = 'subpe/cancel';
    ...
    protected function _getCheckout()
    {
     ...
     //when customer selects subpe payment method
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('subpe')->__('Customer was redirected to subpe.')
                )->save();
            }
            //save order and quote ids
             if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setsubpeQuoteId($session->getQuoteId());
                $session->setsubpeSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setsubpeRealOrderId($session->getLastRealOrderId());
    ...
    public function responseAction()
    {
        ...
        try {
            ...
            $txnstatus = false;
            $authStatus = false;
            $mer_encrypted = Mage::getStoreConfig('payment/subpe_cc/inst_key');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $mer_decrypted= Mage::helper('subpe')->decrypt_e($mer_encrypted,$const);
            
            $merid_encrypted = Mage::getStoreConfig('payment/subpe_cc/inst_id');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $merid_decrypted= Mage::helper('subpe')->decrypt_e($merid_encrypted,$const);
            ...
            $_testurl = NULL;
            $transaction_status_url = Mage::getStoreConfig('payment/subpe_cc/transaction_status_url');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $_testurl= Mage::helper('subpe')->decrypt_e($transaction_status_url,$const);
    ...
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unssubpeRealOrderId();
            $session->setQuoteId($session->getsubpeQuoteId(true));
            $session->setLastSuccessQuoteId($session->getsubpeSuccessQuoteId(true));
            ...
            $this->_debug('subpe error: ' . $e->getMessage());
            Mage::logException($e);
        }
        $this->_redirect('checkout/cart');
    ...
    public function failAction()
    {
        // set quote to active
        $session = $this->_getCheckout();       
        if ($quoteId = $session->getsubpeQuoteId()) {
        ...
        Mage::log("failed");
        $session->addError(Mage::helper('subpe')->__('The order has failed.'));
        $this->_redirect('checkout/cart');
    ...
    public function cancelAction()
    {
        ...
        Mage::log("cancel");
        $session->addError(Mage::helper('subpe')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    ...
    protected function _processSale($request)
    {
        ...
        $order->addStatusToHistory(Mage::getStoreConfig('payment/subpe_cc/order_status'),Mage::helper('subpe')->__('Payment successful through subpe PG'));
        $order->save();
    ...
    protected function _processFail($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATUS_FRAUD, Mage::helper('subpe')->__('Payment failed'));
            $this->_order->save();
        }
        $session = $this->_getCheckout();
        $session->addError(Mage::helper('subpe')->__('It seems some issue in server to server communication. Kindly connect with administrator.'));
        $this->_redirect('checkout/cart');
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('subpe')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('subpe')->getPendingPaymentStatus();
    }
    ...
    public function curltestAction() {
        ...
        if(!function_exists("curl_init")){
            ...
            if(!empty($_GET)){
                ...
                $transaction_status_url = Mage::getStoreConfig('payment/subpe_cc/transaction_status_url');
                $const = (string)Mage::getConfig()->getNode('global/crypt/key');
                $check_status_url= Mage::helper('subpe')->decrypt_e($transaction_status_url,$const);
                $testing_urls = array(
                    Mage::getBaseUrl(),
                    "www.google.co.in",
                    $check_status_url
    ...
    public function ajaxAction() {
        ...
        if(isset($_POST["promo_code"]) && trim($_POST["promo_code"]) != "") {
            ...
            if(Mage::getStoreConfig('payment/subpe_cc/promo_code_local_validation')=='1') {

                $promocode=Mage::getStoreConfig('payment/subpe_cc/promo_codes');
