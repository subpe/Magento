...
class Bpay_subpe_Model_Cc extends Mage_Payment_Model_Method_Abstract

{	
	//unique internal payment method identifier
	
	
	protected $_code = 'subpe_cc';
    ...
    protected $_formBlockType = 'subpe/form';
    protected $_infoBlockType = 'subpe/info';
    protected $_order;
    ...
    {
          return Mage::getUrl('subpe/processing/redirect');
    }
    ...
    {
    		$transaction_url = Mage::getStoreConfig('payment/subpe_cc/transaction_url');
			$const = (string)Mage::getConfig()->getNode('global/crypt/key');
			$this->_liveUrl= Mage::helper('subpe')->decrypt_e($transaction_url,$const);
    ...
    public function getFormFields()
    {
		...
		$const = (string)Mage::getConfig()->getNode('global/crypt/key');// Mage::getStoreConfig('payment/subpe_cc/constsubpe');
		$mer = Mage::helper('subpe')->decrypt_e($this->getConfigData('inst_key'),$const);
		$merid = Mage::helper('subpe')->decrypt_e($this->getConfigData('inst_id'),$const);
		...
		$callbackUrl = rtrim(Mage::getUrl('subpe/processing/response',array('_nosid'=>true)),'/');
       ...
       //for Session
		...
		//for session
            $price=round($price);
			$checksum = Mage::helper('subpe')->getChecksumFromArray($params, $mer,$customer_name);//generate checksum
