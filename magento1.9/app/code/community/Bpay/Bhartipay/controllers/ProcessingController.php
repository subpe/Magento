...
class Bpay_bhartipay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'bhartipay/success';
    protected $_failureBlockType = 'bhartipay/failure';
    protected $_cancelBlockType = 'bhartipay/cancel';
    ...
    protected function _getCheckout()
    {
     ...
     //when customer selects bhartipay payment method
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('bhartipay')->__('Customer was redirected to bhartipay.')
                )->save();
            }
            //save order and quote ids
             if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setbhartipayQuoteId($session->getQuoteId());
                $session->setbhartipaySuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setbhartipayRealOrderId($session->getLastRealOrderId());
    ...
    public function responseAction()
    {
            ...
            $session = $this->_getCheckout();
            $order = Mage::getModel('sales/order');
            //post data after return from pg
        ...
        try {
            ...
            $txnstatus = false;
            $authStatus = false;
            $mer_encrypted = Mage::getStoreConfig('payment/bhartipay_cc/inst_key');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $mer_decrypted= Mage::helper('bhartipay')->decrypt_e($mer_encrypted,$const);
            
            $merid_encrypted = Mage::getStoreConfig('payment/bhartipay_cc/inst_id');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $merid_decrypted= Mage::helper('bhartipay')->decrypt_e($merid_encrypted,$const);
            ...
            if($request['STATUS'] == "Captured" && $request['RESPONSE_CODE']=="000"){
                $txnstatus = true;
            }
            
            $_testurl = NULL;
            $transaction_status_url = Mage::getStoreConfig('payment/bhartipay_cc/transaction_status_url');
            $const = (string)Mage::getConfig()->getNode('global/crypt/key');
            $_testurl= Mage::helper('bhartipay')->decrypt_e($transaction_status_url,$const);

            if($txnstatus && $isValidChecksum){
                // Create an array having all required parameters for status query.
                $requestParamList = array("MID" => $merid_decrypted , "ORDERID" => $request['ORDER_ID']);
                $const = (string)Mage::getConfig()->getNode('global/crypt/key');
                $authStatus = true;
                if($authStatus == false) {
    ...
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsbhartipayRealOrderId();
            $session->setQuoteId($session->getbhartipayQuoteId(true));
            $session->setLastSuccessQuoteId($session->getbhartipaySuccessQuoteId(true));
            ...
            $this->_debug('bhartipay error: ' . $e->getMessage());
            Mage::logException($e);
        }
        $this->_redirect('checkout/cart');
    ...
    public function failAction()
    {
        // set quote to active
        $session = $this->_getCheckout();       
        if ($quoteId = $session->getbhartipayQuoteId()) {
        ...
        Mage::log("failed");
        $session->addError(Mage::helper('bhartipay')->__('The order has failed.'));
        $this->_redirect('checkout/cart');
    ...
    public function cancelAction()
    {
        ...
        Mage::log("cancel");
        $session->addError(Mage::helper('bhartipay')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    ...
    protected function _checkReturnedPost()
    {
        ...
        $this->_order = Mage::getModel('sales/order')->loadByIncrementId($_POST['ORDER_ID']);
    ...
    protected function _processSale($request)
    {
        ...
        $order->addStatusToHistory(Mage::getStoreConfig('payment/bhartipay_cc/order_status'),Mage::helper('bhartipay')->__('Payment successful through bhartipay PG'));
        $order->save();
    ...
    protected function _processFail($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATUS_FRAUD, Mage::helper('bhartipay')->__('Payment failed'));
            $this->_order->save();
        }
        $session = $this->_getCheckout();
        $session->addError(Mage::helper('bhartipay')->__('It seems some issue in server to server communication. Kindly connect with administrator.'));
        $this->_redirect('checkout/cart');
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('bhartipay')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('bhartipay')->getPendingPaymentStatus();
    }
    ...
    public function curltestAction() {
        ...
        if(!function_exists("curl_init")){
            ...
            if(!empty($_GET)){
                ...
                $transaction_status_url = Mage::getStoreConfig('payment/bhartipay_cc/transaction_status_url');
                $const = (string)Mage::getConfig()->getNode('global/crypt/key');
                $check_status_url= Mage::helper('bhartipay')->decrypt_e($transaction_status_url,$const);
                $testing_urls = array(
                    Mage::getBaseUrl(),
                    "www.google.co.in",
                    $check_status_url
    ...
    public function ajaxAction() {
        ...
        if(isset($_POST["promo_code"]) && trim($_POST["promo_code"]) != "") {
            ...
            if(Mage::getStoreConfig('payment/bhartipay_cc/promo_code_local_validation')=='1') {

                $promocode=Mage::getStoreConfig('payment/bhartipay_cc/promo_codes');
